...
const cs = StyleSheet.create({
  ...
  bar: { width: '100%', borderRadius: Layout.buttonRadius, minHeight: 4 },
...
export default function MeteoScreen() {
  ...
  const load = async (isRefresh = false) => {
    ...
    try {
      ...
      const cacheData = {
        ...
        condition: weatherCodeToLabel(data.current.weatherCode)
      };
      await AsyncStorage.setItem('agropilot_meteo_cache', JSON.stringify(cacheData));
      // --------------------------------------
  ...
  return (
    ...
    >
      <View style={[s.header, { paddingTop: insets.top + 22 }]}>
        <View style={s.headerTop}>
          ...
          <View style={s.hTempBlock}>
            <Text style={s.hTempBig}>{Math.round(current.temp)}°</Text>
        ...
        <View style={s.metricsRow}>
          ...
          <View style={s.metricDivider} />
          <MetricBox value={`${Math.round(current.apparent)}°`} label="Ressenti" />
        </View>
      </View>
      ...
      <View style={s.grid}>
        {tab === 'general' && (<>
          <DataCard title="Température" value={`${Math.round(current.temp)}°C`} sub={`Ressenti ${Math.round(current.apparent)}°C`} />
...
const s = StyleSheet.create({
  ...
  frostAlert: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#EEF4FF', borderLeftWidth: 4, borderLeftColor: '#1565C0', marginHorizontal: 16, marginBottom: 14, borderRadius: Layout.cardRadius, paddingHorizontal: 14, paddingVertical: 12 },
  frostIcon: { fontSize: 16 },
  frostAlertTxt: { fontSize: 13, color: '#0D47A1', lineHeight: 18, flex: 1 },
  mapContainer: { marginHorizontal: 16, marginBottom: 14, borderRadius: Layout.cardRadius, overflow: 'hidden', height: 420, ...Layout.cardShadow },
  ...
  dec: { backgroundColor: Colors.white, borderRadius: Layout.cardRadius, padding: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', ...Layout.softShadow },
  ...
  chartCard: { backgroundColor: Colors.white, borderRadius: Layout.cardRadius, padding: 16, ...Layout.softShadow },
  chartToggles: { flexDirection: 'row', gap: 6, marginBottom: 12 },
  ctBtn: { paddingHorizontal: 14, paddingVertical: 7, borderRadius: Layout.buttonRadius + 6, backgroundColor: Colors.background, borderWidth: 1, borderColor: Colors.border },
  ...
  tabBtn: { paddingHorizontal: 16, paddingVertical: 9, borderRadius: Layout.buttonRadius + 6, backgroundColor: Colors.white, borderWidth: 1, borderColor: Colors.border },
  ...
  card: { width: '47.5%', backgroundColor: Colors.white, borderRadius: Layout.cardRadius, padding: 14, gap: 4, ...Layout.softShadow },
  cardHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 },
  cardTitle: { fontSize: 11, color: Colors.textMuted, fontWeight: '600', textTransform: 'uppercase', flex: 1 },
  ...
  fRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.white, borderRadius: Layout.inputRadius, paddingHorizontal: 14, paddingVertical: 11, gap: 10, ...Layout.softShadow },
